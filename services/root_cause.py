...
class RootCauseAnalyzer:
    ...
    def analyze(
        ...
        call_chain: list[str],
        all_logs: list[dict],
        fault_symptom: str,  # 必需：外部传入的简洁版故障现象
        ...
        try:
            ...
            # 字段名转换：后端使用英文字段名，前端期望中文字段名
            matched_cases = [
                {
                    **case,
                    "根因分析": case.get("root_cause", ""),
                    "处置建议": case.get("suggestion", ""),
                }
        ...
        # 3. 设置故障现象
        result["故障现象"] = fault_symptom

        result["matched_cases"] = matched_cases
        result["is_new_case"] = False  # 默认不是新故障，后续根据相似度判定
    ...
    def _extract_log_text(self, logs: list[dict], max_chars: int = 3000) -> str:
        ...
        for log in logs:
            ...
            if msg:  # 只添加有内容的日志（去掉级别标记，保留服务前缀）
                parts.append(f"{svc}: {msg}")
            if stack:
